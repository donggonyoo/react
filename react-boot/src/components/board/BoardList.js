...
const BoardList=()=>{
    ...
    */
    ...
    const getBoardList = ()=>{
        ...
        //백엔드의 cotroller로 queryString을 Map형태로 보냄 
        
        .then((resp)=>resp.json()) //resp(Map)을 json객체로 형변환 후 
        .then((json)=>{ //json객체 이용해 하나씩 세팅함
            setBlist(json.bList);
            setBoardCount(json.boardCount) 
            setStart(json.start) //시작페이지 번호
            setPageInt(json.pageInt)//종료페이지번호
                ...
                <ul className="pagination center" style={{justifyConent:"center"}}>
                    ...
                    {getPage(start,end).map((p)=>(
                        <li key={p} className={pageInt === p ? "page-item active":"page-item"}>
                            <a className="page-link" href={"/board/boardList?pageNum="+p}>{p}</a>
                        </li>
                    ))}
